...
num_coding %>% 
  ...
  pack_rows(., 'Ecological opportunities', 12, 23) %>% 
  pack_rows(., 'Socio-cognitive consquences', 24, 27) %>% 
  pack_rows(., 'Socio-cognitive consquences', 28, 32) 
```
...
# choose variables to perform pPCA on:
variables <- c('Predation risk', 'Mobility D-index', 'Home range size', 'Env. seasonality',
...
```{r AIC_model_comparison_I, results = 'hide'}
...
```{r relationship_social_and_ecological_opps, results = 'hide'}
...
tab_estimates_rel %>% 
  ...
  pack_rows("Social Opportunities", 1, 4) %>%
  pack_rows("Ecological Opportunities", 5, 8)
```
...
driving brain size evolution. In fact, whereas the social opportunity PCs show no effect on brain size (PGLS: N = 67, λ = 0.99, β PC1social = 0.005 (p = 0.114); β PC2social = 0.004 (p = 0.157)), both PCs of the ecological domain show significant effects (PGLS: N = 50, λ = 1.00, β PC1ecology = 0.005 (p = 0.020); β PC2ecology = 0.004 (p = 0.018)). These results show still strong trends when combining the four PCs into a single regression model (PGLS: N = 43, λ = 1.00, β PC1social = 0.0009 (p = 0.814); β PC2social = 0.003 (p = 0.525), β PC1ecology = 0.006 (p = 0.076); β PC2ecology = 0.005 (p = 0.067)).

<br> 



**Phylogenetic PCA social opportunities**
```{r, results='hide'}
...
```{r socoppalt}
# output table PCA
kable_phylo_pca_loadings(pca.soc.opp.alt$PC.loadings[, c(1,2)],
                         N = attributes(pca.soc.opp.alt$PC.loadings)$N)
```
...
**Phylogenetic PCA ecological opportunities**
```{r, results='hide'}
...
```{r ecooppalt}
# output table PCA
kable_phylo_pca_loadings(pca.eco.opp.alt$PC.loadings[,c(1,2)],
                         N = attributes(pca.eco.opp.alt$PC.loadings)$N)
```
...
**Phylogenetic PCA social consequences**
```{r, results='hide'}
...
```{r socconsalt}
# output table PCA
kable_phylo_pca_loadings(data.frame(PC1 = pca.soc.cons.alt$PC.loadings[, c(1)]),
                         N = attributes(pca.soc.cons.alt$PC.loadings)$N)
```

<br>

**Phylogenetic PCA ecological consequences**
```{r, results = 'hide'}
...
```{r ecoconsalt}
# output table PCA
kable_phylo_pca_loadings(data.frame(PC1 = pca.eco.cons.alt$PC.loadings[,1]),
                         N = attributes(pca.eco.cons.alt$PC.loadings)$N)
```
